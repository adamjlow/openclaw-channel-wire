#!/usr/bin/env node
/**
 * Register (or re-authenticate) an openclaw-channel-wire Wire *application* on a Wire backend
 * and emit the WIRE_SDK_* environment block the plugin needs.
 *
 ...
 *   node scripts/register-app.mjs create    --host URL --email ADMIN_EMAIL [--name "OpenClaw"] [--category other]
 *                                           [--description TEXT] [--code 2FA_CODE] [--out FILE] [--force]
...
// backend offers it so this script and the plugin agree on API semantics.
...
Wire cloud host: https://prod-nginz-https.wire.com (self-hosted backends use their own nginz URL)
`);
}
...
async function login(host, v, email, password, code) {
  const body = { email, password, label: "openclaw-channel-wire-register-app" };
...
async function cmdCreate(host, opts) {
  ...
  const body = {
    ...
    description: (opts.description ?? "OpenClaw assistant connected through openclaw-channel-wire.").slice(0, 300),
    password: a.password,
  };
  let created;
  try {
  ...
  stdout.write(
    "\nNext: configure channels.wire with these values (see README), start the gateway, then add the app to a conversation.\n" +
...
async function cmdRefresh(host, opts) {
  ...
  stdout.write(`Issued a new token for app ${appId}. Update channels.wire.apiToken and restart the gateway.\n`);
...
if (!host) fail("--host (or WIRE_SDK_API_HOST) is required, e.g. https://prod-nginz-https.wire.com");
